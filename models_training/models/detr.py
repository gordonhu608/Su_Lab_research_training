...
from .detrseg import (DETRsegm, PostProcessSegm,
...
class DETR(nn.Module):
    ...
    def forward(self, samples):
        if isinstance(samples, (list, torch.Tensor)):
            samples = util.nested_tensor_from_tensor_list(samples)
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        if log:
            ...
            losses['class_error'] = 100 - util.accuracy(src_logits[idx], target_classes_o)[0]
        return losses
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes):
        ...
        target_masks, valid = util.nested_tensor_from_tensor_list(masks).decompose()
